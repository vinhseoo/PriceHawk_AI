# SmartCart AI — Project Blueprint & Technical Documentation (v3)
...
SmartCart AI là nền tảng trợ lý mua sắm thông minh:
...
│   ├── pom.xml                            ← Parent POM
│   ├── smartcart-common/                  ← ★ Shared: DTOs, exceptions, constants, events, utils
│   ├── smartcart-security-starter/        ← ★ Shared: JWT, auth filter, SecurityUtils
│   ├── smartcart-data-starter/            ← ★ Shared: BaseEntity, BaseRepo, JPA auditing
│   ├── smartcart-messaging-starter/       ← ★ Shared: RabbitMQ config, EventPublisher
│   ├── smartcart-gateway/                 ← API Gateway
│   ├── smartcart-user-service/            ← User Service (auth + profile + wishlist)
│   ├── smartcart-catalog-service/         ← Catalog Service
│   └── smartcart-notification-service/    ← Notification Service
│
...
**Task 1.2**: `smartcart-common` — ApiResponse, PageResponse, GlobalExceptionHandler, all Exceptions, Constants, Enums (Platform, SourceType, ScraperTier...), Event DTOs, Utils. Unit tests.
**Task 1.3**: `smartcart-security-starter` — JwtTokenProvider, JwtAuthenticationFilter, SecurityConfig, SecurityUtils.
**Task 1.4**: `smartcart-data-starter` — BaseEntity (UUID + auditing), BaseRepository, JpaAuditingConfig, BaseSpecification.
**Task 1.5**: `smartcart-messaging-starter` — RabbitMQConfig, EventPublisher, AbstractEventConsumer.
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: smartcart_secret
    ports: ["5432:5432"]
    volumes:
  ...
  rabbitmq:
    ...
    environment:
      RABBITMQ_DEFAULT_USER: smartcart
      RABBITMQ_DEFAULT_PASS: smartcart_secret
